...
- **UI primitive components** (`components/ui/`): lowercase filenames (e.g., `button.tsx`, `dialog.tsx`) are acceptable when generated by third-party tooling such as shadcn/ui.
- **React hooks:** `useXxx.ts` camelCase filenames follow React community convention and are acceptable (e.g., `useSettings.ts`, `useRealtimeEntities.ts`).
