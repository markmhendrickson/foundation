    ...
    while IFS= read -r -d '' cmd_file; do
        ...
        if [ -e "$target_file" ]; then
            if [ -L "$target_file" ]; then
                ...
                # Not a regular file, skip
                print_warn "File exists but is not a regular file: $cmd_name (skipping)"
                continue
            else
                # File exists - check if it needs updating
                if [ "$cmd_file" -nt "$target_file" ]; then
                    # Source is newer, update the copy with header comment
                    if [[ "$cmd_file" == *.mdc ]]; then
                    ...
                    print_info "  ✓ Updated $cmd_name (source was newer)"
                    ...
                    if ! head -1 "$target_file" | grep -q "DO NOT EDIT THIS FILE DIRECTLY"; then
                        ...
                        print_warn "File already exists: $cmd_name (skipping to preserve existing file)"
                    fi
                    continue
                fi
            fi
