...
# Manifest-driven selective install: only allowlisted rules/commands (copy mode)
if [ -f "$REPO_ROOT/cursor_rules_manifest.json" ]; then
    print_info "Using cursor_rules_manifest.json (selective install)..."
    mkdir -p .cursor/rules .cursor/commands

    # Clear existing rules and commands so only allowlisted remain (files and symlinks)
    if [ -d ".cursor/rules" ]; then
        find .cursor/rules -maxdepth 1 \( -type f -o -type l \) \( -name "*.mdc" -o -name "*.md" \) -delete
        print_info "  Cleared .cursor/rules"
    fi
    if [ -d ".cursor/commands" ]; then
        find .cursor/commands -maxdepth 1 \( -type f -o -type l \) -name "*.md" -delete
        print_info "  Cleared .cursor/commands"
    fi

    RULES_COPIED=0
    COMMANDS_COPIED=0

    # Foundation rules (slugs: e.g. "agent_constraints" -> foundation/.../agent_constraints.mdc)
    while IFS= read -r slug; do
        [ -z "$slug" ] && continue
        for ext in mdc md; do
            src="$RULES_DIR/${slug}.${ext}"
            if [ -f "$src" ]; then
                target=".cursor/rules/$(basename "$src")"
                copy_mdc_with_comment "$src" "$target" "$src" > "$target"
                print_info "  Copied (foundation) $slug.$ext"
...
import json
try:
    with open('$REPO_ROOT/cursor_rules_manifest.json') as f:
        m = json.load(f)
    for s in m.get('always_on', {}).get('foundation_rules', []) or []:
        print(s)
except Exception:
    pass
")

    # Repo rules (paths relative to repo root; copy to .cursor/rules by basename)
    while IFS= read -r path; do
        [ -z "$path" ] && continue
        src="$REPO_ROOT/$path"
        if [ -f "$src" ]; then
            name=$(basename "$path")
            target=".cursor/rules/$name"
            if [[ "$src" == *.mdc ]]; then
                copy_mdc_with_comment "$src" "$target" "$path" > "$target"
            else
                {
                    echo "<!--"
                    echo "⚠️  DO NOT EDIT THIS FILE DIRECTLY"
                    echo "Copy of: $path"
                    echo "Run '/setup_cursor_copies' to sync."
                    echo "-->"
                    echo ""
                    cat "$src"
                } > "$target"
            fi
            print_info "  Copied (repo) $path"
            RULES_COPIED=$((RULES_COPIED + 1))
        fi
    done < <(python3 -c "
import json
try:
    with open('$REPO_ROOT/cursor_rules_manifest.json') as f:
        m = json.load(f)
    for p in m.get('always_on', {}).get('repo_rules', []) or []:
        print(p)
except Exception:
    pass
")
...
try:
    with open('$REPO_ROOT/cursor_rules_manifest.json') as f:
        m = json.load(f)
    for c in m.get('commands', {}).get('foundation_commands', []) or []:
        print(c)
except Exception:
    pass
"); do
        [ -z "$slug" ] && continue
        for ext in md; do
            src="$COMMANDS_DIR/${slug}.${ext}"
            [ -f "$src" ] || src="$COMMANDS_DIR/$slug"
            if [ -f "$src" ]; then
                target=".cursor/commands/$(basename "$src")"
                { echo "<!-- Copy of: $src -->"; echo ""; cat "$src"; } > "$target"
                print_info "  Copied (command) $slug"
...
try:
    with open('$REPO_ROOT/cursor_rules_manifest.json') as f:
        m = json.load(f)
    for p in m.get('commands', {}).get('repo_commands', []) or []:
        print(p)
except Exception:
    pass
"); do
        [ -z "$path" ] && continue
        src="$REPO_ROOT/$path"
        [ -f "$src" ] && cp "$src" ".cursor/commands/$(basename "$path")" && print_info "  Copied (repo command) $path" && COMMANDS_COPIED=$((COMMANDS_COPIED + 1))
    done

    print_info "Manifest install complete: $RULES_COPIED rules, $COMMANDS_COPIED commands."
    exit 0
fi

# If foundation is a git submodule, use symlinks instead of copies (stays in sync with submodule updates)
if [ "$FOUNDATION_DIR" = "foundation" ] && git submodule status foundation 1>/dev/null 2>&1; then
    print_info "Foundation is a git submodule; using symlinks (setup_cursor_rules)..."
    SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
    exec bash "$SCRIPT_DIR/setup_cursor_rules.sh"
fi
...
for rule_file in "$RULES_DIR"/*.mdc; do
    if [ -f "$rule_file" ]; then
        ...
        if [ -e "$target_file" ]; then
            rm "$target_file"
            print_info "  ✓ Replaced $rule_name with copy from source"
        fi
...
for cmd_file in "$COMMANDS_DIR"/*.md; do
    if [ -f "$cmd_file" ]; then
        ...
        if [ -e "$target_file" ]; then
            rm "$target_file"
            print_info "  ✓ Replaced $cmd_name with copy from source"
        fi
...
if [ -d "docs" ]; then
    ...
    while IFS= read -r -d '' rule_file; do
        ...
        if [ -e "$target_file" ]; then
            rm "$target_file"
            print_info "  ✓ Replaced $symlink_name with copy from source"
        fi
        if [[ "$rule_file" == *.mdc ]]; then
            copy_mdc_with_comment "$rule_file" "$target_file" "$rule_file" > "$target_file"
        else
    ...
    # Process repo commands from docs/ directory
    print_info ""
    print_info "Processing repository commands from docs/ directory..."
    REPO_COMMANDS_REMOVED=0
    REPO_COMMANDS_COPIED=0
    
    # Remove existing repo command files (ending in _command.md, excluding foundation-* and foundation_*)
    if [ -d ".cursor/commands" ]; then
        for existing_cmd in .cursor/commands/*_command.md; do
            if [ -e "$existing_cmd" ]; then
                basename_file=$(basename "$existing_cmd")
                # Skip if it starts with foundation- or foundation_
                if [[ ! "$basename_file" =~ ^foundation[-_] ]]; then
                    rm "$existing_cmd"
                    print_info "  ✓ Removed repo command: $basename_file"
                    REPO_COMMANDS_REMOVED=$((REPO_COMMANDS_REMOVED + 1))
                fi
            fi
        done
    fi
    
    # Find all *_command.md files in docs/ directory (recursively)
    # Prefer .mdc over .md if both exist
    while IFS= read -r -d '' cmd_file; do
        # Get relative path from docs/ directory
        rel_path="${cmd_file#docs/}"
        # Get directory path and filename
        dir_path=$(dirname "$rel_path")
        # Remove .mdc or .md extension
        if [[ "$rel_path" == *.mdc ]]; then
        ...
        # Determine file extension based on source file
        if [[ "$cmd_file" == *.mdc ]]; then
            file_ext=".mdc"
        else
            file_ext=".md"
        fi
        
        # Create command name by removing _command suffix and replacing / with _
        # Example: docs/finalize_chat_command.md -> finalize_chat.md
        base_cmd_name="${file_name%_command}"
        if [ "$dir_path" = "." ]; then
            cmd_name="${base_cmd_name}${file_ext}"
        else
            # Replace / with _ in path
            path_prefix=$(echo "$dir_path" | tr '/' '_')
            cmd_name="${path_prefix}_${base_cmd_name}${file_ext}"
        fi
        ...
        if [[ "$cmd_file" == *.mdc ]]; then
            copy_mdc_with_comment "$cmd_file" "$target_file" "$cmd_file" > "$target_file"
        else
            {
                echo "<!--"
                echo "⚠️  DO NOT EDIT THIS FILE DIRECTLY"
                ...
                echo ""
                echo "To edit this command:"
                echo "1. Edit the source file: $cmd_file"
                echo "2. Run '/setup_cursor_copies' to sync changes"
                echo ""
                echo "This file will be overwritten when running setup_cursor_copies."
                echo "-->"
        ...
        print_info "  ✓ Copied $cmd_file -> $cmd_name"
        REPO_COMMANDS_COPIED=$((REPO_COMMANDS_COPIED + 1))
    done < <(find docs -type f \( -name "*_command.mdc" -o -name "*_command.md" \) -print0 | while IFS= read -r -d '' file; do
        # If both .mdc and .md exist, prefer .mdc
        base="${file%.mdc}"
        base="${base%.md}"
        if [ -f "${base}.mdc" ] && [ -f "${base}.md" ] && [[ "$file" == *.md ]]; then
    ...
    if [ $REPO_COMMANDS_COPIED -eq 0 ] && [ $REPO_COMMANDS_REMOVED -eq 0 ]; then
        print_info "  No repo commands found in docs/ directory"
    fi
else
    print_info "  No docs/ directory found, skipping repo rules and commands"
fi

print_info ""
if [ $RULES_COPIED -gt 0 ] || [ $COMMANDS_COPIED -gt 0 ] || [ $RULES_REMOVED -gt 0 ] || [ $COMMANDS_REMOVED -gt 0 ] || [ $REPO_RULES_COPIED -gt 0 ] || [ $REPO_RULES_REMOVED -gt 0 ] || [ $REPO_COMMANDS_COPIED -gt 0 ] || [ $REPO_COMMANDS_REMOVED -gt 0 ]; then
    ...
    if [ $REPO_COMMANDS_REMOVED -gt 0 ]; then
        print_info "  Repo commands removed: $REPO_COMMANDS_REMOVED"
    fi
    ...
    if [ $REPO_COMMANDS_COPIED -gt 0 ]; then
        print_info "  Repo commands copied: $REPO_COMMANDS_COPIED"
    fi
    print_info ""
