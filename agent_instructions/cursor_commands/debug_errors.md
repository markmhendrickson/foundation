...
```bash
/debug_errors [action|target-repo-name]
```
...
```bash
/debug_errors start      # Start the error debugging watcher
/debug_errors stop       # Stop the running watcher
/debug_errors status     # Check watcher status
/debug_errors logs       # Show watcher logs
/debug_errors restart    # Restart the watcher
```
...
```bash
/debug_errors                    # Check current repo's pending errors (default)
/debug_errors neotoma            # Check sibling repo's pending errors
/debug_errors personal-project   # Check another sibling repo
```

**Note:** To check watcher status, use `/debug_errors status`. The default behavior (no arguments) checks for pending errors in the current repository.
...
```bash
# Watcher management
/debug_errors start      # Start watcher
/debug_errors status     # Check watcher status
/debug_errors logs        # View watcher logs
/debug_errors stop        # Stop watcher

# Error debugging
/debug_errors                    # Check current repo's pending errors
/debug_errors neotoma            # Check sibling repo's pending errors
/debug_errors --list-only       # List errors without debugging
/debug_errors --all --auto      # Process all errors automatically
```
...
4. **Also show pending errors count:**
   ...
   - If errors exist, suggest running `/debug_errors` to debug
...
Use /debug_errors to debug pending errors.
```
...
```bash
/debug_errors --list-only
/debug_errors neotoma --list-only
```
...
- Use `/debug_errors start` to start the watcher
- Use `/debug_errors status` to check watcher and pending errors
...
Use /debug_errors to debug highest priority error.
```
...
```bash
/debug_errors logs
/debug_errors logs --follow  # Tail logs
```
...
- `/manage_error_debugging` - **Deprecated**: Use `/debug_errors [action]` instead
  - `/manage_error_debugging start` → `/debug_errors start`
  - `/manage_error_debugging stop` → `/debug_errors stop`
  - `/manage_error_debugging status` → `/debug_errors status`
  - `/manage_error_debugging logs` → `/debug_errors logs`
  - `/manage_error_debugging restart` → `/debug_errors restart`
